...
#if (SMART_SOUNDBOX_EN)
u32 fw_report_firmware_license(void)
{
    u32 err = RCSP_OP_ERR_NONE;
    u32 offset = 0;
	...
	printf("BT_SMART_LICENCE_STRING_LEN = %d\n", BT_SMART_LICENCE_STRING_LEN);
	printf_buf((u8 *)BT_SMART_LICENCE_STRING, BT_SMART_LICENCE_STRING_LEN);
	offset = offset + rcsp_fill_packet_frame_func(data_boday + offset, "LICE", FRAME_TYPE_ANSI, (u8 *)BT_SMART_LICENCE_STRING, BT_SMART_LICENCE_STRING_LEN);
#else
	u8 *license_buf;	
	u16  license_len;
	license_buf = user_get_auth_code(&license_len);
	printf("license_len = %d\n", license_len);
	if(license_len == 0 || license_len == 0xffff)
	{
		return 	RCSP_OP_ERR_PARAM;	
	}

	printf_buf(license_buf, license_len);
	offset = offset + rcsp_fill_packet_frame_func(data_boday + offset, "LICE", FRAME_TYPE_ANSI, license_buf, license_len);
...
#if 0
///this is an examle
///自定义数据回复处理, 自定义数据发送参考AC690x_智能音箱介绍文档第二章关于基于rcsp自定义数据发送接口说明
extern u8 rcsp_user_data[100];
extern u32 rcsp_user_data_len;
u32 fw_report_user_data(void)
{
	...
	offset = offset + rcsp_fill_packet_frame_func(data_boday + offset, "USER", FRAME_TYPE_ANSI, (u8*)rcsp_user_data, rcsp_user_data_len);
    rcsp_fill_packet_head_func((void*)data_pt, 0xffff, offset,1);
...
static u32 app_public_cmd_parse(u8 *OperationData,u8 *data,u16 len)
{
...
#if (SMART_SOUNDBOX_EN)
	case CBW_CMD_GET_LICENSE:
		err = fw_report_firmware_license();
		break;
#endif//SMART_SOUNDBOX_EN

#if 0//this is an example
	case CBW_CMD_USER_DATA:
		err = fw_report_user_data();
		break;
#endif
    ...
    default:
#if (SMART_SOUNDBOX_EN)
		err = bt_smart_rcsp_command_recieve(OperationData, data, len);
#else
