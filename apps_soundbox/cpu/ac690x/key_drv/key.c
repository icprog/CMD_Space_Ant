...
volatile u8 g_powerdown_flag = 0;
...
*/
/*----------------------------------------------------------------------------*/
#define KEY_DOUBLE_CLICK_CNT    50//35 //35*10ms
void key_scan(void *ptr)
{
    static tu8 last_key = NO_KEY;
    static tu8 double_click_last_key = NO_KEY;
    ...
    if(key_press_flag)
    {
        ...
        if(key_press_flag > KEY_DOUBLE_CLICK_CNT)
        {
            puts(" One_Click -----------------------------------\n");
            key_press_flag = 0;
            key2msg_filter(KEY_SHORT_UP, double_click_last_key);
        }
    }
    ...
    if(!strcmp(keymsg_task_name,"btmsg"))//蓝牙模式才有按键双击功能
    {
        ...
        {
			if(double_click_last_key != back_last_key && key_press_flag != 0) 
			{
				key_press_flag = 0;
			}
            ...
            if(key_press_flag == 0)
			{
				double_click_last_key = back_last_key;	
                key_press_flag = 1;
			}
            ...
            {
                printf(" Double_Click ---------2---------------- === %d\n", back_last_key);
                key_press_flag = 0;
                /* key2msg_filter(key_status, 8); */
				if(keymsg_task_name)
				{
					key_msg_sender(keymsg_task_name, BT_Double_Click_msg_tab[back_last_key]);
				}
            }
        }
        else
